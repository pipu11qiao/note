...
标准输入流一般负责从用户向程序传递数据。那些需要标准输入的程序一般通过一些设备，如键盘来接收输入。标准输入遇到EOF（end of file）就会停止，就像它的名字，EOF说明没有要读取的数据了。


可以通过运行**cat**来查看标准输入的行为。cat命令代表串联，意思是连接或者混合一些文件。通常用来拼接两个文件。当你只输入cat命令本身，它会循环调用提示命令（prompt)
...
```
当你敲入一个数字并按下回车的时候，你就是在向cat程序发送标准输入，也被称作输入。相应的，cat程序会将你的输入当做标准输出展示。
...
当你不带参数运行ls，它会展示文件夹内的所有内容。如果传入一个文件夹名当参数，将会列举提供的文件夹内的内容。
...
管道(pipe)命令用来让流从一个程序到另一个程序。当一个程序的标准输出被发送到另一个程序时，第一个程序的产生的被另外程序接收到的数据不会在终端展示。只有被第二个程序返回的过滤后的数据会展示。
...
```
这组命令将**ls**展示当前文件夹内容的输出输送到**less**程序,less将发送给他的数据以每一行作为一个新入口单独显示。
...
上述命令会将你系统的root文件夹中的内容作为标准输出将其写入名称为root_dir_contents的文件中。它会删除原先的文件内容，因为用的是单个尖括号

###### 命令> /dev/null

/dev/null 是一个特殊的文件用来丢弃发送给他的任意数据，它被用于丢弃没用的或者会妨碍一个程序或命令功能的标准输出。所有的被发送到/dev/null的输出都会被忽略。
...
```
这个命令会搜索 /var文件夹以及它的所有子级文件的文件名和扩展名，并匹配字符串deb,返回这些文件的路径，并将匹配的内容红色高亮显示
...
学习使用Linux内置的重定向指令会让人有点气馁，但是当你完成该教程后你会很好地掌握该技能。现在你已经了解重定向和管道基本的工作原理，你将有能力涉足使用shell脚本世界，那将会是该教程中强调的程序和表达式更加的便捷。
